...
async function check() {
  ...
  if (tables.rows.some(r => r.table_name === 'restaurants')) {
    const restaurants = await pool.query(`SELECT id, diet_type_id, name FROM restaurants ORDER BY name`);
  ...
  if (tables.rows.some(r => r.table_name === 'menu_items')) {
    ...
    const sample = await pool.query(`SELECT id, name, restaurant_id, available, is_deleted FROM menu_items LIMIT 3`);
